...
router.post("/add", upload.array("images"), async (req, res) => {
    ...
    response(res, async () => {
        ...
        await product.save();
        res.json({ message: "Product save is successfully!" });
    });
});
...
router.post("/removeById", async (req, res) => {
    response(res, async () => {
        ...
        res.json({ message: "Product remove is successfully" });
    })
});
...
//Change Product Avtive OR Passive
...
router.post("/changeActiveStatus", async (req, res) => {
    response(res, async () => {
        ...
        res.json({ message: "Product status is successfully changed!" })
    })
});
...
router.post("/removeImageByProductIdAndIndex", async (req, res) => {
    response(req, async () => {
        ...
        if (product.imageUrls.length == 1) {
            res.json({ message: "You can not delete last image! product have a must one image!!" });
        }
        else {
            ...
            await Product.findByIdAndUpdate(_id, product);
            fs.unlink(image.path, () => { });
            res.json({ message: "Image remove is successfully!" });
        }
    })
});
